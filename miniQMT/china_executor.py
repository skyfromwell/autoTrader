...
def _connect() -> None:
    global _trader, _acc
    _trader = XtQuantTrader("", 1, callback=_CB())
...
def _place_order(order: dict) -> dict:
    ...
    order_id = _trader.order_stock(
        ...
        order_remark  = f"signal@{signal_price}",
    )
    log.info(f"BUY {xt_sym}  {volume}sh @ mkt  order_id={order_id}  est=¥{volume*px:,.0f}")
    return {"symbol": xt_sym, "volume": volume, "order_id": order_id, "exec_price_est": px}
...
def run() -> None:
    ...
    while True:
        ...
        for fpath in order_files:
            ...
            log.info(f"Processing {pair}  queued_at={order.get('queued_at','?')}")

            try:
                result = _place_order(order)
            except Exception as e:
                log.error(f"{pair} order failed: {e} — leaving file for retry")
                continue

            if result.get("order_id", -1) == -1:
                log.error(f"{pair} rejected (order_id=-1) — leaving file for retry")
                continue

            # Success — delete file; Syncthing propagates deletion to Mac/Mini
            try:
                fpath.unlink()
                log.info(f"✅ {pair}  vol={result['volume']}  order={result['order_id']}  "
                         f"→ file deleted, sync will propagate")
            except Exception as e:
