...
Runs the SSE/SZSE scan, generates a formatted PDF, sends to Telegram.
"""
...
def generate_pdf(results: list[dict], pdf_path: str) -> str:
    ...
    _section("〰️  Mixed / Choppy", mix_df, ALT_ROW)
...
def send_to_telegram(pdf_path: str):
    ...
    caption = f"📊 China A-Share SMA10/21 Scan — {datetime.now().strftime('%Y-%m-%d %H:%M')}"
